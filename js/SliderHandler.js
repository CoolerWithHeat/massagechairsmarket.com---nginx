...
$(document).ready(function() {
    ...
    const price_filter = getUrlParams();
    const currentMin = price_filter.minPrice || 0
    const currentMax = price_filter.maxPrice || 15999
    const formattedMin = `$${currentMin}`;
    const formattedMax = `$${currentMax}`;
    ...
    function AdjustPriceSlider(event, max) {
        if (max) {
            const sliderMin = document.getElementsByName('min-value');
            ...
            const desired_price = event.target.value;
            const counterpart_price = moneyFormat.from(sliderMax.value);
            const concluded_price = moneyFormat.from(desired_price);
    ...
    function RangerHandler(values, handle) {
      if (MaxPriceRepresentor && MinPriceRepresentor){
          ...
          const formattedMin = moneyFormat.from(minPrice);
          ...
          if (handle){
              ...
              const event = new Event('onchange');
              MinPriceRepresentor.dispatchEvent(event);
            }
          document.getElementsByName('min-value').value = formattedMin;
