...
function PlacePhoneCallButton(no_bootstrap=false){
    ...
    if (ButtonWindow){
        ButtonWindow.innerHTML = `
                <div onclick="HandleCallAction();" class="${no_bootstrap ? 'PhoneCallWindow-noBootstrap' : 'PhoneCallWindow'} bubbly-button">
...
const ShowContactForm = (closeTabFunction, no_bootstrap=false)=>{
    ...
    if (window){
        ...
        setTimeout(() => {PlacePhoneCallButton(no_bootstrap);}, 99);
    }
};
...
function GetContactForm(RemoveTab=null, no_bootstrap=false){
    const window = document.getElementById('MainContactWindow');
    if (window){
        ...
        importCSS('/static/contactCSS/contactMain.css', ()=>ShowContactForm(RemoveTab, no_bootstrap))
